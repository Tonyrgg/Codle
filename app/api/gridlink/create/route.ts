...
export async function POST() {
  ...
  let code = makeCode();
  for (let i = 0; i < 4; i++) {
    const { data: exists } = await admin.from("gridlink_matches").select("id").eq("code", code).maybeSingle();
    if (!exists) break;
    code = makeCode();
  }
  ...
  if (mErr || !match) return NextResponse.json({ ok: false, error: mErr?.message || "Create failed" }, { status: 500 });
  ...
  const { error: iErr } = await admin.from("gridlink_inventory").insert(invRows);
